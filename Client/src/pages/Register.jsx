...
const Register = () => {
  ...
  try {
    if (!showOTP) {
      await register(name, email, password); // AuthContext wala register chalega
      setShowOTP(true); // OTP input show hoga
    } else {
      await verifyOTP(email, otp); // OTP verify hoga
      navigate("/dashboard"); // success ke baad dashboard
    }
  } catch (err) {
    console.log("Register error:", err); // asli error console mein dikhega
    console.log("Backend error:", err.response?.data); // backend response, agar aaya ho

    setError(err.response?.data?.message || err.message || "Registration failed");
    // UI mein readable error dikhayega
  } finally {
    setLoading(false);
  }
};
